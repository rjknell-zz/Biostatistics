...
We start out by loading the data which I have on a web server

```{r}
quolls <- read.csv("https://github.com/rjknell/Biostatistics/raw/main/data/quolls.csv")
```

Now to draw the scatterplot
...
The data set malawi_carbon contains data on two lung immunity measures (oxidative burst activity and phagocytic activity) from people exposed to chronic carbon particulates in their homes in Malawi. These data were originally published by Rylance *et al.* (2015)^1^ as part of a study of how exposure to smoke from cooking fires affects lung function. Let's load the data and have a look at how these two variables relate to particulate exposure.

```{r}
malawi_carbon <- read.csv("https://github.com/rjknell/Biostatistics/raw/main/data/malawi_carbon.csv")
```
...
```{r scatterplot01, exercise = TRUE, exercise.lines = 8, exercise.setup = "prepare-carbon"}
...
```{r scatterplot02, exercise = TRUE, exercise.lines = 8, exercise.setup = "prepare-carbon"}
...
Now we'll use a dataframe loaded called `worldbank` which has data for 186 countries from 2014 as published by the World Bank. Let's load the data into R.

```{r}

worldbank <- read.csv("https://github.com/rjknell/Biostatistics/raw/main/data/worldbank.csv")
```
...
```{r scatterplot1, exercise = TRUE, exercise.lines = 8, exercise.setup = "prepare-worldbank"}
...
The `carnivores` data frame contains brain mass and body mass data for 199 species of carnivorous mammals, originally published as part of a larger dataset by Burger *et al.* in 2019^1^. The variable for brain mass is called `Mean_brain_mass_g` and that for body mass is `Mean_body_mass_Kg`. Plot a scatterplot of brain mass against body mass and have a look at it. This plot is better with open symbols so use the default option: you can remove the `pch = 16` line. This time I'll let you load the data from https://github.com/rjknell/Biostatistics/raw/main/data/carnivores.csv
...
#To load the data you need to use the "read.csv()" function. 
#Allocate the output of that function to an object called carnivores.
#
...
carnivores <- read.csv("https://github.com/rjknell/Biostatistics/raw/main/data/carnivores.csv")
...
```{r echo = FALSE}
## Here because data from an exercise doesn't stay in the workspace
...
```{r prepare-carnivores}
carnivores <- read.csv("http://research.sbcs.qmul.ac.uk/r.knell/learnr_data/carnivores.csv")
```

```{r scatterplot3, exercise = TRUE, exercise.lines = 10, exercise.setup = "prepare-carnivores"}
...
#To load the data frame, just use the same code
#as before with the names changed
...
mammal_longevity <- 
  read.csv(
  "https://github.com/rjknell/Biostatistics/raw/main/data/mammal_longevity.csv"
)
...
```{r prepare-mammals}
##Here because data loaded in an exercise doesn't stay in the workspace
...
```{r scatterplot5, exercise.lines = 25, exercise = TRUE, exercise.setup = "prepare-mammals"}
