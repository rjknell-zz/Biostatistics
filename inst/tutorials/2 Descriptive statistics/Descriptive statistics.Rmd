...
# Load data
height <- read.csv(
  "http://research.sbcs.qmul.ac.uk/r.knell/learnr_data/body_height_immunity.csv"
  )

# Draw the histogram
hist(height$lysozyme,
     col = "aquamarine4",
     main = "",
...
In the `height` data frame there is another variable called "phagocytic" which is a measure of the phagocytic response by the leucocytes for each person in the study. Calculate the mean and the median for this variable.
...
```{r mean3, exercise = TRUE, exercise.setup = "prepare-height"}

```

```{r mean3-hint-1}
#As for the lysozyme data, we have to
#tell R to look in the height data frame 
...
mean(height$phagocytic, na.rm = TRUE)
median(height$phagocytic, na.rm = TRUE)
```
...
```{r mean4, exercise = TRUE, fig.height = 5, fig.width =4, exercise.setup = "prepare-height"}
...
#the height data frame, so you have to refer
#to it as height$phagocytic
#
...
```{r spread1, exercise = TRUE, exercise.setup = "prepare-height"}

```

```{r spread1-hint-1}
#Don't forget that lysozyme is a variable within
#the height data frame, so you need to give the
...
```{r spread2, exercise = TRUE, exercise.setup = "prepare-height"}
...
```{r spread3, exercise = TRUE, exercise.setup = "prepare-height"}
...
```{r fig.width = 4, fig.height = 8, echo = FALSE, fig.cap = "**Figure 3** Frequency histograms for lysozyme activity (A) and phagocytic activity (B). The vertical lines show the quartiles and medians and the shaded boxes show the interquartile ranges."}
